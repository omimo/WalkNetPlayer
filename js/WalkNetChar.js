var WalkNetChar = WalkNetChar || {};


WalkNetChar = function(n, jm, bm, jg, bg) {
	this.name = n;
	...
	this.loadFromURL = function(url, callback) {
		...
		reader = new MocapParsers.bvhParser(this.name + "READER");
		this.url = url;
	...
	this.onHeaderReceived = function(data) {
		...
		headerReader = new MocapParsers.bvhStreamParser();
	...
	this.loadFromStream = function(url, callback) {
		...
		self.webSocket.onmessage = function(event) {
			...
			} else if (messageLines[0].startsWith("$ACTS$")) {
				if (self.webSocket.onmessageActs)
					self.webSocket.onmessageActs(event);
			}
